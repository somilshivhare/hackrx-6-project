"""
LLM Answerer module using GPT-4 for generating answers from document chunks.
...
class LLMAnswerer:
    """Handles GPT-4 based answer generation from document chunks"""
    ...
    async def generate_answer(
        ...
        try:
            ...
            if not response:
                raise ValueError("Failed to generate response from GPT-4")
    ...
    def _create_prompt(self, question: str, context: str) -> str:
        """Create optimized prompt for GPT-4 with enhanced accuracy"""
...
- Structure complex answers in a clear, logical manner"""
    ...
    async def _call_gpt4(self, prompt: str) -> Optional[str]:
        """Call GPT-4 API with retry logic and enhanced error handling"""
        max_retries = 3
        retry_delay = 1
            ...
            try:
                ...
                response = await loop.run_in_executor(
                    ...
                    lambda: self.client.chat.completions.create(
                        model=self.model,
                        messages=[
                            {"role": "system", "content": "You are an expert insurance and legal document analyst with deep expertise in policy interpretation."},
                ...
                print(f"❌ Error calling GPT-4 (attempt {attempt + 1}/{max_retries}): {str(e)}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                else:
                    print(f"❌ Failed to call GPT-4 after {max_retries} attempts")
                    return None
        
        return None
    
    def _parse_response(self, response: str, chunks: List[str], full_document: str) -> Dict[str, str]:
        """Parse GPT-4 response and extract structured components"""
        try:
